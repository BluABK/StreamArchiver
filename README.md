...
- The main table has a **🖴 drive column**, and the toolbar's **🖴 Drives menu mutes whole drive letters**: untick a drive and its rows vanish from the table and the headline count (shown as `(+N on muted drives)` instead) — for **parking a flaky disk's backlog** until the hardware improves, without it polluting the actionable list. Muted drives are remembered across restarts, and — unlike the text/type filters — the **bulk buttons skip muted rows**: *Delete all* / *Clear all failed* never touch a parked drive. Un-mute the drive to see and act on its rows again.
